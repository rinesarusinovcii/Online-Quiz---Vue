...
const formData = reactive({
  ...
  questionId: { val: null, isValid: true }
});
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await ChoiceService.createChoice(choice);
  ...
  <the-main-layout>
    <template #content>
      <div class="col-lg-8 col-md-8 col-12">
        <quiz-card>
          <template #header>
            <h2 class="text-center">New Choice</h2>
          </template>
          <template #default>
            <form @submit.prevent="handleSubmit">
              ...
              <div class="mb-1">
                ...
                <select
                    ...
                    v-model="formData.questionId.val"
                    @blur="clearValidity('questionId')"
                >
                  <option disabled value=null>Select a question</option>
                  <option v-for="question in questions" :key="question.value" :value="question.value">
                    {{ question.label }}
                  </option>
                </select>
              </div>
